...
class ProxyPresenter extends ConfiguratorPresenter
{
    public function actionWrite($model)
    {
        ...
        try {
            echo json_encode(array('success' => $modelObject->interceptAndSave()));
        } catch (Exception $e) {
    ...
    public function actionDelete($model, $id)
    {
        ...
        try {
            ...
            echo json_encode(array('success' => false, 'error' => $e->getMessage(), 'e' => $e->getMessage()));
        }
        $this->terminate();
    }
    ...
    public function actionRead($model, $returnType = 'JSON')
    {
        try {
            ...
            foreach ($modelObject->getColumns() as $column) {
                ...
                if (!is_null($constraint = $column->getConstraint()) && $column->grid() && !array_key_exists(
                        $column->getName(),
                        $columnData
                    )
                ) {
                    ...
                    $columnData[$column->getName()] = ModelStorage::getModelByModelClass(
                        $constraint->model
            ...
            if ($returnType == 'JSON') {
                echo $this->getParameter('callback') . '(' . json_encode(
                        array('totalCount' => 1, 'items' => array(array('customer_id' => $e->getMessage())))
                    ) . ')';
            }
            throw $e;
        }
    ...
    public function convertQuery($q)
    {
        ...
        return strtr($q, $prevodni_tabulka);
    }
}
